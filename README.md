...
Herramientas compartidas por todos los talleres de Sistemas Operativos
(Exactas, UBA). Hoy son las de la VM; el repo está pensado para que después
convivan otras.
...
| `vm.mk` | cátedra | Los targets de make genéricos (`install`, `start-vm`, ...) y las canned recipes que comparten los talleres. Lo incluye el `Makefile` de cada uno. |
| `cloud-init/` | — | Plantilla de configuración del guest: usuario, clave ssh, montaje 9p, paquetes. |
| `VM.md` | alumnos | La documentación del entorno. El `README.md` de cada taller la linkea. |

## Cómo funciona

Todo se compila y se ejecuta adentro de una VM Debian x86_64, no en la máquina
...
1. Una imagen **base**, que se descarga y provisiona una sola vez **por
   máquina** y queda cacheada en `~/.local/share/taller-so`, compartida entre
   todos los talleres.
...
> todos los talleres.** La imagen base la hornea el primer taller que corra
...
Una sola rama, `main`, y los talleres la siguen siempre. **Un push acá le llega
